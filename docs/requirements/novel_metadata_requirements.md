...
novels (
    ...
    isbn VARCHAR(20),
    age_rating ENUM('ALL', 'TEEN', 'MATURE', 'ADULT'),
    original_platform VARCHAR(100), -- 원본 연재 플랫폼
    external_url VARCHAR(500), -- 외부 플랫폼 링크
...
novel_status (
    novel_id BIGINT PRIMARY KEY,
    status ENUM('PREPARING', 'ONGOING', 'SIDE_STORY', 'COMPLETED', 'HIATUS', 'DISCONTINUED'),
    total_episodes INT, -- 총 화수
    latest_episode INT, -- 최신 화수
    update_frequency VARCHAR(20), -- 연재 주기 ('DAILY', 'WEEKLY', 'BIWEEKLY', 'MONTHLY', 'IRREGULAR')
    published_at TIMESTAMP, -- 연재 시작일
    completed_at TIMESTAMP, -- 완결일
    last_updated_at TIMESTAMP, -- 마지막 업데이트 일시
    estimated_next_update TIMESTAMP, -- 예상 다음 업데이트
    platform_specific_data JSON, -- 플랫폼별 추가 정보
    created_at TIMESTAMP,
    updated_at TIMESTAMP,
